...
def evaluate(args):
    ...
    with graph.as_default() as g:
        with tf.name_scope("create_test_inputs"):
            ...
            test_image, test_score,test_mean_std = tf.expand_dims(test_image, dim=0), tf.expand_dims(test_score,dim=0) , tf.expand_dims(test_mean_std,dim=0)# Add one batch dimension.
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for i in range(len(test_reader.image_list)):
            ...
            loss_ , scores_hat_= sess.run([loss,scores_hat], feed_dict={imgs: image_, scores: score_, means:mean_std_})
            # mean,std=scores_stats(scores_hat_)
            mean = mean_score(scores_hat_)
            score_set.append(mean)
            if i % 50 ==0:
                logger.info("test image:{}/{}, true_mean_mos/predict_mos is {}/{},the emd loss: {}.".format(i,len(test_reader.image_list),mean_std_[:,0],mean,loss_))
...
def main():
    ...
    global logger
    logger = setup_logger("TF_NIMA_evaluating", output_dir,"evaluate_")
    logger.info(args)
