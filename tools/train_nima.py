...
from loss.EMD_loss import emd_loss,_emd
...
def process_command_args():
    ...
    parser.add_argument('--pretrain_weights',type=str,default=os.path.abspath('..')+"/data/vgg_models/"+'vgg16_weights.npz')
    ...
    parser.add_argument('--dataset',default='tid2013',type=str,choices=["LIVE", "CSIQ", "tid2013"],help='datset choice')
...
def train(args):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        summary_writer = tf.summary.FileWriter(os.path.join(args.logs_dir,'train/{}-{}'.format(args.exp_name,timestamp)), sess.graph,filename_suffix=args.exp_name)
        ...
        for step in range(args.iter_max):
            ...
            means_out, mean_hat_out,emd_loss_,l2_loss_,total_loss_, _ = sess.run([means,mean_hat,emd_loss_out,l2_loss, loss, optimizer],feed_dict={imgs:image_batch_,scores:label_batch_,means:mean_std_batch_,lr:base_lr})

            if step % iters_per_epoch==0:
                logger.info("step %d/%d, the emd loss is %f,l2_loss is %f,total loss is %f, time %f,learning rate: %lf" % (step,args.iter_max,emd_loss_,l2_loss_,total_loss_, (time.time() - start_time),base_lr))
                summary_str = sess.run(summary_op,feed_dict={imgs:image_batch_,scores:label_batch_,means:mean_std_batch_,lr:base_lr})
                # print(means_out.reshape(-1,))
                # print(mean_hat_out)
                srocc, krocc, plcc, rmse, mse = evaluate_metric(means_out.reshape(-1,), mean_hat_out)
                logger.info("evaluate train batch SROCC_v: %.3f\t KROCC: %.3f\t PLCC_v: %.3f\t RMSE_v: %.3f\t mse: %.3f\n" % (srocc, krocc, plcc, rmse, mse))
